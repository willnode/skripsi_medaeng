...
def proses_teks(contoh, contoh_id, kosakata_id, index, proses_kelas_kata):
  ...
  try:
    ...
    while i < len(contoh):
      ...
      # Karakter adalah kurung lengkung "("?
        ...
        # Teknis: Hanya proses apabila memproses dalam contoh bahasa Madura
        if proses_kelas_kata and word in ['a.', 'Adv.', 'n.', 'Num.', 'P.', 'Pron.', 'Pron persona tunggal.', 'Pron persona pertama jamak.', 'Pron persona pertama tunggal.', 'v.', 'Ling.']:
          pos = word[:-1]
    ...
    for kosakata_text in input_text.readlines():
      ...
      for contoh_idx, contoh in enumerate(split_contoh_list_by_dot_comma(kosakata_text)):
        ...
        for index, contoh in enumerate(split_teks_list_by_comma(rawtext_mad)):
          ...
          proses_teks(contoh.strip(), contoh_id,
                kosakata_id, index + 1, True)

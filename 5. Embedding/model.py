...
class NEG_loss(nn.Module):
  def __init__(self, num_classes, embed_size, weights=None, device='cpu'):
    """
    ...
    self.out_embed.weight = Parameter(t.FloatTensor(\
        self.num_classes, self.embed_size).uniform_(-1, 1))

    self.in_embed = nn.Embedding(self.num_classes, self.embed_size)
    self.in_embed.weight = Parameter(t.FloatTensor(\
        self.num_classes, self.embed_size).uniform_(-1, 1))
  ...
  def forward(self, input_labes, out_labels, num_sampled):
    ...
    input = self.in_embed(input_labes.repeat(1, window_size)\
          .contiguous().view(-1))
    ...
    ''' ∑[batch_size * window_size, num_sampled, embed_size] *
      [batch_size * window_size, embed_size, 1] ->
      ∑[batch_size, num_sampled, 1] -> [batch_size] '''
    sum_log_sampled = t.bmm(noise, input.unsqueeze(2))\
        .sigmoid().log().sum(1).squeeze()

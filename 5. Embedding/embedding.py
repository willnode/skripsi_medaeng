...
def print_config():
  print(CONTEXT_WINDOW, NEGATIVE_SAMPLE, \
      BATCH_SIZE, LEARNING_RATE, EPOCH, DEVICE)
...
AND tb_token.lang = ? ORDER BY tb_teks_token."index"''', [lang]):
  ...
  last_token = 0
  for row in cur.execute('''
SELECT t."index" index1, t2."index" index2 FROM
tb_teks_token_neighbour ttn, tb_token t, tb_token t2
...
AND t.lang = ? AND ttn.distance < ? ORDER BY ttn.id''', [lang, CONTEXT_WINDOW]):
  ...
  neighbor_input, neighbor_output = zip(*[[x, y] \
      for n in neighbor_list for x, y in n.items()])
  return (
    np.array(token_list, dtype=dutf8), # token array
    np.array([[0, *teks_list.get(x, []), 1] for x in \
      range(MAX + 1)], dtype=dvari), # teks array
    (np.array(neighbor_input), np.array(neighbor_output)), # neighbour array
  )


def data_gen(input, output):
  dataloader_input = DataLoader(torch.from_numpy(input)\
      .to(DEVICE), batch_size=BATCH_SIZE, shuffle=False)
  dataloader_output = DataLoader(torch.from_numpy(output)\
      .to(DEVICE), batch_size=BATCH_SIZE, shuffle=False)
...
def train_embedding(token, neigbour, dim, data, log):
  ...
  model = NEG_loss(TOKEN_LENGTH, EMBEDDING_SIZE, \
      device=DEVICE).train().to(DEVICE)

  optim = Adam(model.parameters(), lr=LEARNING_RATE)
  scheduler = LinearLR(optim, start_factor=1, end_factor=0, \
      total_iters=EPOCH * len(dataloader))
  epoch = 0
  if data is not None:
...
def load_and_train(db, output):
  ...
  data = {
    ...
    'TEKS': np.swapaxes(np.array([
        [np.pad(x, (0, max_h_teks - len(x)), \
          constant_values=2) for x in mad[1][:max_v_teks]],
        [np.pad(x, (0, max_h_teks - len(x)), \
          constant_values=2) for x in ind[1][:max_v_teks]],
      ], dtype=np.int32), 0, 1),
  }
  ...
  for dim in [256, 512]:
    data['MAD']['embedding'][dim] = train_embedding(mad[0], mad[2], \
        dim, data['MAD']['embedding'][dim], output + '.log')
    data['IND']['embedding'][dim] = train_embedding(ind[0], ind[2], \
        dim, data['IND']['embedding'][dim], output + '.log')
